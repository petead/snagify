...
const plans = [
  { slug: "starter", name: "Starter", price: 149, credits: 10, extraCredit: 18, users: "1", usersLabel: "inspector", popular: false, color: "#9A88FD", priceId: "price_1TC1CrKIsjOh5d33lCuUGmcd" },
  { slug: "growth", name: "Growth", price: 249, credits: 20, extraCredit: 15, users: "3", usersLabel: "inspectors", popular: true, color: "#7C3AED", priceId: "price_1TC1D3KIsjOh5d33oEd1E3T1" },
  { slug: "agency", name: "Agency", price: 349, credits: 30, extraCredit: 13, users: "∞", usersLabel: "inspectors", popular: false, color: "#1E1B4B", priceId: "price_1TC1DPKIsjOh5d33hlQhhUTf" },
] as const;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#F8F7F4] pb-32">
      ...
      <AnimatePresence>
        {selected > 0 && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="mx-5 mt-3 px-4 py-3 rounded-xl bg-green-50 border border-green-100 flex items-center gap-2">
            ...
            <p className="text-xs text-green-700 font-medium">{selected === 1 ? `Save AED ${18 - 15}/cr vs Starter on extra credits` : `Save AED ${18 - 13}/cr vs Starter on extra credits`}</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#F8F7F4]/95 backdrop-blur-sm px-5 pb-8 pt-4 border-t border-gray-100">
        ...
        <p className="text-center text-[10px] text-gray-400 mt-2">Cancel anytime · Unused credits roll over monthly</p>
      </div>
    </div>
  );
}
